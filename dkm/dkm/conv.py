...
class ConvMixup(nn.Module):
    def __init__(self, in_ind_points, out_ind_points, filter_size=3, stride=1):
        super().__init__()
    ...
    def forward(self, K):
        return StructMat(*t.utils.checkpoint.checkpoint(self._forward, *K))
...
class NystromGAP(nn.Module):
    def forward(self, K):
        ...
        gKt = t.sum((gKti @ pos_inv(K.ii)) * gKti, -1)
        assert gKt.shape == (batch,)

        D = K.t - t.sum((K.ti @ pos_inv(K.ii)) * K.ti, -1)
        assert D.shape == (batch, *space)
...
class NystromGAPMixup(nn.Module):
    def __init__(self, in_ind_points, out_ind_points):
        super().__init__()
...
class BigFinalConvMixup(nn.Module):
    ...
    def forward(self, K):
        ...
        if not self.initialised:
            ...
            self.latent_learned_weights = nn.Parameter(t.randn(self.out_ind_points, math.prod(space) * self.in_ind_points).to(K.t.device))
...
def ResNetBlock(n, in_ind_points, out_ind_points, dof, filter_size=3, stride=1, bn_indnorm="local", bn_tnorm="local", bn_indscale="global", bn_tscale="global", Kernel=ReluKernel, mult_eps=1e-4, abs_eps=1e-4):
